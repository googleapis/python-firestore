...
class FirestoreAdminClient(metaclass=FirestoreAdminClientMeta):
    ...
    def backup_schedule_path(project: str,database: str,backup_schedule: str,) -> str:
        ...
        return "projects/{project}/databases/{database}/backupSchedules/{backup_schedule}".format(project=project, database=database, backup_schedule=backup_schedule, )

    @staticmethod
    def parse_backup_schedule_path(path: str) -> Dict[str,str]:
        """Parses a backup_schedule path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/databases/(?P<database>.+?)/backupSchedules/(?P<backup_schedule>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def operation_path(project: str,database: str,operation: str,) -> str:
        ...
        return "projects/{project}/databases/{database}/operations/{operation}".format(project=project, database=database, operation=operation, )

    @staticmethod
    def parse_operation_path(path: str) -> Dict[str,str]:
        """Parses a operation path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/databases/(?P<database>.+?)/operations/(?P<operation>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def user_creds_path(project: str,database: str,user_creds: str,) -> str:
        """Returns a fully-qualified user_creds string."""
        return "projects/{project}/databases/{database}/userCreds/{user_creds}".format(project=project, database=database, user_creds=user_creds, )

    @staticmethod
    def parse_user_creds_path(path: str) -> Dict[str,str]:
        """Parses a user_creds path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/databases/(?P<database>.+?)/userCreds/(?P<user_creds>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = FirestoreAdminClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = FirestoreAdminClient._read_environment_variables()
        self._client_cert_source = FirestoreAdminClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = FirestoreAdminClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[FirestoreAdminTransport], Callable[..., FirestoreAdminTransport]] = (
    ...
    def clone_database(self,
        ...
        routing_param_regex = re.compile('^projects/[^/]+/databases/(?P<database_id>[^/]+)(?:/.*)?$')
        regex_match = routing_param_regex.match(request.pitr_snapshot.database)
