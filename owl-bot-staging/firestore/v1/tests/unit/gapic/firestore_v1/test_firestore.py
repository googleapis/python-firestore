...
async def test_partition_query_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.partition_query(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_collection_ids_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_collection_ids(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_list_documents_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("mask", "orderBy", "pageSize", "pageToken", "readTime", "showMissing", "transaction", )) & set(("parent", "collectionId", )))
