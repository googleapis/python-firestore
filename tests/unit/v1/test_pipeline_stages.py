...
class TestGenericStage:
    ...
    @pytest.mark.parametrize("input_args,expected_params", [
        (("name",), []),
        ...
        (("n",Constant.of(True), Constant.of(False)), [Value(boolean_value=True), Value(boolean_value=False)]),
        (("n", Constant.of(GeoPoint(1, 2))), [Value(geo_point_value={"latitude": 1, "longitude": 2})]),
        (("n", Constant.of(None)), [Value(null_value=0)]),
        (("n", Constant.of([0,1,2])), [Value(array_value={"values": [Value(integer_value=n) for n in range(3)]})]),
        (("n", Value(reference_value="/projects/p/databases/d/documents/doc")), [Value(reference_value="/projects/p/databases/d/documents/doc")]),
        (("n", Constant.of({"a": "b"})), [Value(map_value={"fields": {"a": Value(string_value="b")}})]),
    ])
    ...
    def test_repr_with_reference(self):
        """
        reference_value can't properly be displayed without knowing the 
        """
