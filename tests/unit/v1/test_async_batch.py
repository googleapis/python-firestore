...
class TestAsyncWriteBatchWrite(aiounittest.AsyncTestCase):
    ...
    @pytest.mark.asyncio
    async def _write_helper(self, retry=None, timeout=None):
        from google.cloud.firestore_v1 import _helpers
        from google.cloud.firestore_v1.types import firestore
        from google.cloud.firestore_v1.types import write

        # Create a minimal fake GAPIC with a dummy result.
        firestore_api = AsyncMock(spec=["batch_write"])
    ...
    async def test_write(self):
        await self._write_helper()

    @pytest.mark.asyncio
    async def test_write_w_retry_timeout(self):
        from google.api_core.retry import Retry

        retry = Retry(predicate=object())
        timeout = 123.0

        await self._write_helper(retry=retry, timeout=timeout)

    @pytest.mark.asyncio
    async def test_write_as_context_mgr_wo_error(self):
        from google.cloud.firestore_v1.types import firestore
        from google.cloud.firestore_v1.types import write
        ...
        self.assertEqual(batch.write_results, list(write_response.write_results))
        # Make sure batch has no more "changes".
        self.assertEqual(batch._write_pbs, [])
    ...
    @pytest.mark.asyncio
    async def test_write_as_context_mgr_w_error(self):
        firestore_api = AsyncMock(spec=["batch_write"])
        client = _make_client()
        client._firestore_api_internal = firestore_api
        batch = self._make_one(client)
        ...
        firestore_api.batch_write.assert_not_called()
